module ApplicationHelper
  ...
  def show_net_value_link
    ...
    total_loan_lixi_twd = Property.total_loan_lixi.to_i # 所有贷款含利息
    flow_subtract_loan_twd = flow_assets_twd - total_loan_lixi_twd # 流动性扣除贷款
    month_growth_rate =
    raw "<span id=\"properties_net_value_twd\" title=\"流动性资产总值：#{flow_assets_twd}\n比特币资产总值：#{btc_value_twd}\n可买币资产总值：#{investable_fund_twd}\n所有贷款含利息：#{total_loan_lixi_twd}\n流动性扣除贷款：#{flow_subtract_loan_twd}\">#{link_to(@properties_net_value_twd.to_i, chart_path, target: :blank)}</span>|<span id=\"properties_net_value_cny\" title=\"资产净值(#{t(:cny)})\">#{@properties_net_value_cny.to_i}</span>|<span title=\"流动性资产扣除贷款的净值(¥#{(flow_subtract_loan_twd*Property.new.twd_to_cny).to_i}) | #{t(:begin_profit_price)}:#{Property.begin_profit_price.to_i}\">#{flow_subtract_loan_twd}</span>&nbsp;<span id=\"net_growth_ave_month\" title=\"#{$net_start_date}起资产净值增加/月(台币｜人民币)\">#{to_n(@properties_net_growth_ave_month/10000.0,1)}|#{to_n(@properties_net_growth_ave_month_cny/10000.0,1)}</span>|<span title=\"#{Property.cal_year_profit}\">#{link_to(to_n(Property.ave_month_growth_rate,2)+'%',trial_lists_path)}</span>"
  end
