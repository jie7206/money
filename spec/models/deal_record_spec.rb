...
RSpec.describe '模型测试(DealRecord)', type: :model do
  ...
  describe '基本验证' do
    ...
    specify '#165[模型层]成交价、成交量、止盈额度、止损额度必须是正数否则显示错误讯息' do
      ...
      expect_field_value_must_greater_than_zero :deal_record, :earn_limit, $deal_record_earn_limit_type_err
      expect_field_value_must_greater_than_zero :deal_record, :loss_limit, $deal_record_loss_limit_type_err
    end
