...
# 数据库位置
DB = test_db_path()
DB_Local = test_db_path(local=True)
try:
...
def batch_sell_process(test_price, price, base_price, ftext, time_line, u2c, profit_goal, max_sell_count):
    ...
    if max_sell_count > 0 and usdt_now < sell_max_usd:
        ...
        if len(rows) > 0:
            ...
            profit_goal = (1+(price-base_price)**2/10000)*profit_goal
            for row in rows:
                id = row[0]
                amount = row[1]
                created_at = row[2]
...
def last_sell_time():
    global acc_id
    rows = select_db(
        "SELECT created_at FROM deal_records WHERE account = '%s' and auto_sell = 1 and real_profit != 0 and real_profit != 0.01 ORDER BY created_at DESC LIMIT 1" % acc_id)
    return datetime.strptime(rows[0][0], "%Y-%m-%d %H:%M:%S")
            ...
            if price_now > 0:
                ...
                if test_price > 0 or FORCE_BUY == True:
                    ...
                    if FORCE_SELL == True:
                        ...
                        print("NOT Match Sell Conditions!")
                        return every_sec
            else:
                # Can't get price, wait 3 seconds for next operate"
                return 3
        else:
            str = "Already reach target amount, Invest PAUSE!"
            print(str)
                    ...
                    for remaining in range(int(code), 0, -1):
                        ...
                        if remaining % detect_sec == 0:
                            ...
                            if below_price > 0:
                                ...
                                sys.stdout.write("\n")
                                if max_buy_level > 0 and price_now > 0 and price_now < below_price and over_buy_time:
                                    FORCE_BUY = True
                                    break
                            #################################################################
                            if buy_price_period > 0:
                                ...
                                if price_now > 0 and min_price > 0:
                                    ...
                                    sys.stdout.write("\n")
                                    if max_buy_level > 0 and min_price_in_wish and over_buy_time:
                                        FORCE_BUY = True
                                        break
                            ################################################################
